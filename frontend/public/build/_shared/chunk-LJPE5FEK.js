...
var PayComponent = (props) => {
  const { token, swapState, setSwapState: setSwapState2 } = props;
  const { poolToken, assetTokens } = swapState;
  const handleAmountChange = (amount) => {
    assetTokens[token.address].amount = amount;
    setSwapState2({ poolToken, assetTokens });
  };
...
// app/components/Dashboard/Swap/index.tsx
var Swap = (props) => {
  const [swapState, setSwapState2] = (0, import_react2.useState)(props);
  const { poolToken, assetTokens } = swapState;
  ...
  const getQuotes = (swapState2) => {
    ...
    setSwapState2({ poolToken: poolToken2, assetTokens: assetTokens2 });
    return quotes;
  };
  const totalAllocation = Object.values(assetTokens).reduce((total, { allocation }) => total + allocation, 0);
  const handleSwap = () => {
    const quotes = getQuotes(swapState);
    console.log(quotes);
  };
...
//# sourceMappingURL=/build/_shared/chunk-LJPE5FEK.js.map
