...
var Swap = (props) => {
  ...
  const swap = async (address2, payTokens, receiveTokens) => {
    ...
    const amounts = payTokens.map((t) => {
      console.log(t);
      return BigNumber.from((t.amount * 10 ** t.decimals).toLocaleString("fullwide", { useGrouping: false }));
    });
    console.log(amounts);
    const receiveAddresses = receiveTokens.map((t) => t.address);
    const allocations = receiveTokens.map((t) => {
      console.log(t.allocation);
      return BigNumber.from((t.allocation * 10 ** 18).toLocaleString("fullwide", { useGrouping: false }));
    });
...
//# sourceMappingURL=/build/_shared/chunk-KFZWOMA3.js.map
