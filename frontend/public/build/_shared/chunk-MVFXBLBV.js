...
var Swap = (props) => {
  ...
  const postTradePrice = (token, newPoolTokens) => {
    return token.weight * newPoolTokens / (token.reserve + token.amount);
  };
  const assetAmountIn = (tokens, newPoolTokens) => {
    return tokens.filter((address) => address !== contractAddress).map((address) => {
      const token = assetTokens[address];
      const newAssetTokens = token.reserve + token.amount;
      return token.amount * token.weight * newPoolTokens / newAssetTokens;
    }).reduce((p, c) => p + c);
  };
  const getQuotes = () => {
    ...
    const assetAmountIn2 = payTokensSelected.filter((address) => address !== contractAddress).map((address) => {
      const token = assetTokens[address];
      return token.amount * postTradePrice(token, newPoolTokens);
    }).reduce((p, c) => p + c);
    ...
    if (amountIn <= 0)
      throw "Payment must be positive.";
...
//# sourceMappingURL=/build/_shared/chunk-MVFXBLBV.js.map
