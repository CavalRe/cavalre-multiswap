...
var TokenSelect = (props) => {
  ...
  const handleSelect = (v) => {
    ...
    const nSelected = selectedTokens.length;
    console.log(nSelected);
    if (!isPay && nSelected) {
      let totalAllocation = 0;
      for (let i = 0; i < nSelected - 1; i++) {
        totalAllocation += selectedTokens[i].allocation;
      }
      assetTokens[selectedTokens[nSelected - 1].token_address].allocation = Math.max(0, 100 - totalAllocation);
    }
...
var Swap = (props) => {
  ...
  const handleAllocationChange = () => {
    setTotalAllocation(selectedReceiveTokens.map((token) => token.allocation).reduce((p, c) => p + c, 0));
  };
...
//# sourceMappingURL=/build/_shared/chunk-LHFWVIUP.js.map
