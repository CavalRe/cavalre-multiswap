...
var TokenSelect = (props) => {
  ...
  const selectedTokens = items.filter((a) => isPay ? a.token.isPay : a.token.isReceive);
  const handleSelect = (v) => {
    const selected = [...v];
    if (selected.includes(poolToken.address)) {
      console.log("Selection includes the pool token");
      selected.splice(selected.indexOf(poolToken.address), 1);
    ...
    if (nSelected) {
      Object.values(assetTokens).forEach((a) => {
        if (selected.includes(a.address)) {
          a.isPay = isPay;
          a.isReceive = !isPay;
        } else {
          if (isPay) {
      ...
      if (isPay) {
        for (let i = 0; i < nSelected; i++) {
          const oldAmount = assetTokens[selected[i]].amount;
          assetTokens[selected[i]].amount = oldAmount == 0 ? 1 : oldAmount;
        }
      } else {
        let totalAllocation = 0;
        for (let i = 0; i < nSelected - 1; i++) {
          totalAllocation += assetTokens[selected[i]].allocation;
        }
        assetTokens[selected[nSelected - 1]].allocation = Math.max(0, 100 - totalAllocation);
      }
      ;
    }
    ;
...
//# sourceMappingURL=/build/_shared/chunk-4XAHEFGD.js.map
