...
var Swap = (props) => {
  ...
  const swap = async (address2, payTokens, receiveTokens) => {
    ...
    const amounts = payTokens.map((t) => BigNumber.from(10).pow(t.decimals).mul(t.amount));
    console.log(amounts);
    const receiveAddresses = receiveTokens.map((t) => t.address);
    const allocations = receiveTokens.map((t) => BigNumber.from(10).pow(18).mul(t.allocation));
...
//# sourceMappingURL=/build/_shared/chunk-CZLL24AG.js.map
