...
var Swap = (props) => {
  ...
  const getQuote = (swapState2) => {
    ...
    if (poolToken2.allocation !== 0) {
      ...
      console.log(`pool factor: ${factor}`);
      ...
      totalAmountOut = getAssetAmountIn(selectedAssetPayTokens, poolTokens) + poolTokensOut;
      console.log("-------------------------------");
    ...
    const quotes = selectedAssetReceiveTokens.map((token) => {
      ...
      console.log(`${token.name} value out: ${(1 - token.fee) * allocation * totalAmountOut}`);
      const factor = (1 - token.fee) * allocation * totalAmountOut / (token.weight * poolTokens);
      console.log(`${token.name} factor: ${factor}`);
...
//# sourceMappingURL=/build/_shared/chunk-FGZUE7CP.js.map
