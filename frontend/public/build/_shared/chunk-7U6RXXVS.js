  ...
  const setToken = (v, token, totalAllocation) => {
    if (isPay) {
      ...
      if (v.includes(token.address)) {
        ...
        token.allocation = token.allocation == 0 ? Math.max(0, 100 - totalAllocation) / 100 : Math.min(token.allocation, 100 - totalAllocation) / 100;
      } else {
        token.isReceive = false;
      }
      ;
...
var Swap = (props) => {
  ...
  const getPostTradePrice = (asset, newPoolTokens2) => {
    return asset.weight * newPoolTokens2 / (asset.reserve + asset.amount);
  };
  const getAssetAmountIn = (assets, newPoolTokens2) => {
    return assets.reduce((total, asset) => total + asset.amount * getPostTradePrice(asset, newPoolTokens2), 0);
  };
  const getQuote = (swapState2) => {
    ...
    const assetAmountInPreAlloc = getAssetAmountIn(selectedAssetPayTokens, poolTokensPreAlloc);
    const totalAmountInPreAlloc = assetAmountInPreAlloc + (poolToken2.isPay ? poolToken2.amount : 0);
    console.log("-------------------------------");
    console.log("Before...");
    console.log(`poolTokensPreAlloc: ${poolTokensPreAlloc}`);
    console.log(`assetAmountInPreAlloc: ${assetAmountInPreAlloc}`);
    console.log(`totalAmountInPreAlloc: ${totalAmountInPreAlloc}`);
    if (poolToken2.allocation !== 0) {
      const factor = 1 / poolToken2.allocation - getAssetAmountIn(selectedAssetPayTokens, 1);
      ...
      const poolTokens = poolToken2.outstanding + poolTokensOut;
      const assetAmountIn2 = getAssetAmountIn(selectedAssetPayTokens, poolTokens);
      totalAmountIn = assetAmountIn2 - poolTokensOut;
    }
    ;
    console.log("-------------------------------");
    console.log("After...");
    console.log(`newPoolTokens: ${newPoolTokens}`);
    console.log(`assetAmountIn: ${assetAmountIn}`);
    ...
    console.log(`Old pool tokens: ${poolToken2.outstanding}`);
    console.log([
      {
...
//# sourceMappingURL=/build/_shared/chunk-7U6RXXVS.js.map
