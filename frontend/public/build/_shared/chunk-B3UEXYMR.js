...
var Swap = (props) => {
  ...
  const [selectedPayTokens, setSelectedPayTokens] = (0, import_react2.useState)(Object.values(assetTokens).filter((t) => t.isPay).map((t) => t.token_address));
  const [selectedReceiveTokens, setSelectedReceiveTokens] = (0, import_react2.useState)(Object.values(assetTokens).filter((t) => t.isReceive).map((t) => t.token_address));
  ...
  const getQuotes = () => {
    ...
    ;
    return selectedReceiveTokens.map((address) => {
      const token = assetTokens[address];
  ...
  const receiverComponent = (props2) => {
    const { token, onNumberChange } = props2;
    const handleAllocationChange = (allocation) => {
      assetTokens[token.token_address].allocation = allocation;
      onNumberChange && onNumberChange(Object.values(assetTokens).map((t) => t.allocation).reduce((p, c) => p + c));
    };
    return /* @__PURE__ */ React.createElement(Card, {
      radius: "md",
      mt: "xs"
...
//# sourceMappingURL=/build/_shared/chunk-B3UEXYMR.js.map
