...
var TokenSelect = (props) => {
  ...
  const handleSelect = (v) => {
    ...
    if (selected2.includes(poolToken.address)) {
      ...
      poolToken.allocation = poolToken.allocation == 0 ? 100 : poolToken.allocation;
    } else {
      if (isPay) {
    ...
    console.log(`# assets selected = ${nSelected}`);
...
var Swap = (props) => {
  ...
  const getQuote = (swapState2) => {
    ...
    const selectedPayTokens = poolToken2.isPay ? [poolToken2, ...selectedAssetPayTokens] : selectedAssetPayTokens;
    const selectedAssetReceiveTokens = Object.values(assetTokens2).filter((asset) => asset.isReceive);
    const selectedReceiveTokens = poolToken2.isReceive ? [poolToken2, ...selectedAssetReceiveTokens] : selectedAssetReceiveTokens;
...
//# sourceMappingURL=/build/_shared/chunk-2EO45ED6.js.map
