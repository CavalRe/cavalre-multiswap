...
var TokenSelect = (props) => {
  const { title, assetTokens, tokenComponent, isPay, placeholder } = props;
...
var Swap = (props) => {
  ...
  const [selectedState2, setSelectedState2] = (0, import_react2.useState)({
    payTokensSelected: [],
  ...
  const getQuotes = () => {
    const { payTokensSelected, receiveTokensSelected } = selectedState2;
    const poolToken = assetTokens[contractAddress];
    let newPoolTokens = poolTokens + poolToken.amount;
    let assetAmountIn = getAssetAmountIn(payTokensSelected, newPoolTokens);
    let totalAmountIn = assetAmountIn - poolToken.amount;
    if (poolToken.allocation !== 0) {
      const factor = (1 - poolToken.fee) * poolToken.allocation - getAssetAmountIn(payTokensSelected, 1);
    ...
    ;
    return receiveTokensSelected.map((address) => {
      const token = assetTokens[address];
...
//# sourceMappingURL=/build/_shared/chunk-CFIB7E2C.js.map
