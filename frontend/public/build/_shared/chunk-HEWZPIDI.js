  ...
  const setToken = (v, token, totalAllocation) => {
    if (isPay) {
      ...
      if (v.includes(token.address)) {
        ...
        token.allocation = token.allocation == 0 ? Math.max(0, Math.min(token.allocation, 100 - totalAllocation)) : token.allocation;
      } else {
        token.isReceive = false;
      }
      ;
...
//# sourceMappingURL=/build/_shared/chunk-HEWZPIDI.js.map
