...
var TokenSelect = (props) => {
  ...
  const handleSelect = (v) => {
    ...
    ;
    if (!poolToken.isPay && !poolToken.isReceive) {
      poolToken.amount = 0;
      poolToken.allocation = 0;
    }
    Object.values(assetTokens).forEach((asset) => {
      if (isPay) {
        if (v.includes(asset.address)) {
          asset.isPay = true;
          asset.isReceive = false;
        } else {
          asset.isPay = false;
        }
        ;
      ...
      ;
      if (!asset.isPay && !asset.isReceive) {
        asset.amount = 0;
        asset.allocation = 0;
      }
      ;
...
//# sourceMappingURL=/build/_shared/chunk-BRZWWI3M.js.map
