...
var Swap = (props) => {
  ...
  const payerComponent = (props2) => {
    const { token } = props2;
    const [amount, setAmount] = (0, import_react.useState)(token.amount);
    const handleAmountChange = (amount2) => {
      let copiedSwapState = { ...swapState };
      copiedSwapState.assetTokens[token.token_address].amount = amount2;
    ...
    return /* @__PURE__ */ React.createElement(Card, {
      ...
      hideControls: true,
      value: swapState.assetTokens[token.token_address].amount,
  ...
  const receiverComponent = (props2) => {
    const { token } = props2;
    const [allocation, setAllocation] = (0, import_react.useState)(token.allocation);
    const handleAllocationChange = (allocation2) => {
      let copiedSwapState = { ...swapState };
      copiedSwapState.assetTokens[token.token_address].allocation = allocation2;
      copiedSwapState.totalAllocation = Object.values(copiedSwapState.assetTokens).map((a) => a.allocation).reduce((p, c) => p + c);
      setSwapState(copiedSwapState);
      setAllocation(allocation2);
    };
    return /* @__PURE__ */ React.createElement(Card, {
      radius: "md",
      mt: "xs"
...
//# sourceMappingURL=/build/_shared/chunk-LIODIWMD.js.map
