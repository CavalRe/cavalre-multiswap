...
var Swap = (props) => {
  ...
  const preTradePrice = (token) => {
    return poolTokens * token.weight / token.reserve;
  };
  const postTradePrice = (token) => {
    const poolToken = assetTokens[contractAddress];
    const newPoolTokens = poolTokens + poolToken.amount;
    const newAssetTokens = token.reserve + token.amount;
    return token.weight * newPoolTokens / newAssetTokens;
  };
  const getQuotes = () => {
    const { payTokensSelected, receiveTokensSelected } = selectedState;
    const unstakeAmount = payTokensSelected.includes(contractAddress) ? assetTokens[contractAddress].amount : 0;
    const amountIn = payTokensSelected.filter((address) => address !== contractAddress).map((address) => {
      const token = assetTokens[address];
      return token.amount * price(token);
    }).reduce((p, c) => p + c);
    if (unstakeAmount > amountIn)
      return [];
    const allocations = receiveTokensSelected.map((address) => {
      const allocation = assetTokens[address].allocation / 100;
    });
  };
  ...
  (0, import_react.useEffect)(() => {
    const totalAllocation2 = Object.values(assetTokens).map((a) => a.allocation).reduce((p, c) => p + c);
    setTotalAllocation(totalAllocation2);
    console.log(totalAllocation2);
  }, []);
...
var Dashboard = (props) => {
  ...
  const numeraires = [poolToken, ...(_a = Object.values(assetTokens)) == null ? void 0 : _a.map((a) => ({ name: a.name, symbol: a.symbol, price: price2(a.token_address) }))];
  const numeraireMap = {};
  ...
  const rows = (_b = Object.values(assetTokens)) == null ? void 0 : _b.filter((a) => a.reserve > 0).map((a, i) => /* @__PURE__ */ React.createElement("tr", {
    ...
    size: cellTextSize
  }, (price2(a.token_address) / numeraire.price).toLocaleString(void 0, numberOptions))), /* @__PURE__ */ React.createElement("td", {
    align: "right"
  }, /* @__PURE__ */ React.createElement(Text, {
    size: cellTextSize
...
//# sourceMappingURL=/build/_shared/chunk-M5DTSQ54.js.map
