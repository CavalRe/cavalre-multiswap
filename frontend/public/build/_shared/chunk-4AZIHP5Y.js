...
var DashboardHeader = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = (0, import_react.useState)(isAuthenticated);
...
var TokenItem = (0, import_react2.forwardRef)(({ label, value, token, ...others }, ref) => {
...
var Swap = (props) => {
  ...
  const [swapState, setSwapState] = (0, import_react4.useState)(props);
  const { poolToken, assetTokens } = swapState;
  const [isAuthenticated, setIsAuthenticated] = (0, import_react4.useState)(moralis.User.current() !== null);
  ...
  const swap = async (address2, payTokens, receiveTokens) => {
    ...
    const amounts = payTokens.map((t) => BigNumber.from((t.amount * 10 ** t.decimals).toString()));
    console.log(amounts);
    const receiveAddresses = receiveTokens.map((t) => t.address);
    const allocations = receiveTokens.map((t) => {
      console.log(t.allocation);
      return BigNumber.from((t.allocation * 10 ** 18).toString());
    });
  ...
  const handleLogin = async () => await moralis.authenticate().then(() => setIsAuthenticated(true));
...
var Dashboard = (props) => {
  ...
  const [_, setTotalBalance] = (0, import_react5.useState)(0);
  ...
  const [numeraire, setNumeraire] = (0, import_react5.useState)(poolTokenNumeraire);
...
//# sourceMappingURL=/build/_shared/chunk-4AZIHP5Y.js.map
