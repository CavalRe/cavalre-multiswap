...
var Swap = (props) => {
  ...
  const handleAllocationChange = () => {
    setTotalAllocation(Object.values(assetTokens).reduce((total, { allocation }) => total + allocation, 0));
  };
  ...
  const getAssetAmountIn = (tokens, newPoolTokens) => {
    return tokens.filter((token) => token.address !== contractAddress).map((token) => {
      return token.amount * getPostTradePrice(token, newPoolTokens);
    }).reduce((p, c) => p + c, 0);
  };
  const getQuotes = () => {
    console.log("Checking quote...");
    console.log(selectedPayTokens.length);
    console.log(selectedReceiveTokens.length);
    console.log(totalAllocation);
    if (selectedPayTokens.length == 0 || selectedReceiveTokens.length == 0 || totalAllocation !== 100)
      return;
    ...
    setSelectedReceiveTokens({ ...selectedReceiveTokens });
    return quotes;
  };
  const handleSwap = () => {
...
var Dashboard = (props) => {
  ...
  const poolToken = { name: "Pool AssetToken", symbol: "P", price: 1 };
  ...
  const numeraires = [poolToken, ...(_a = Object.values(assetTokens)) == null ? void 0 : _a.map((a) => ({ name: a.name, symbol: a.symbol, price: price(a.address) }))];
  const numeraireMap = {};
  ...
  const rows = (_b = Object.values(assetTokens)) == null ? void 0 : _b.filter((a) => a.reserve > 0).map((a, i) => /* @__PURE__ */ React.createElement("tr", {
    ...
    size: cellTextSize
  }, (balance(a.address) / numeraire.price).toLocaleString(void 0, numberOptions))) : null, /* @__PURE__ */ React.createElement("td", {
    align: "right"
  }, /* @__PURE__ */ React.createElement(Text, {
    size: cellTextSize
  }, (price(a.address) / numeraire.price).toLocaleString(void 0, numberOptions))), /* @__PURE__ */ React.createElement("td", {
    align: "right"
  }, /* @__PURE__ */ React.createElement(Text, {
    size: cellTextSize
...
//# sourceMappingURL=/build/_shared/chunk-DYO7SHAE.js.map
