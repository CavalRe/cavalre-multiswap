...
var PayComponent = (props) => {
  const { token, assetTokens, onNumberChange: onNumberChange2 } = props;
  const handleAmountChange = (amount) => {
    assetTokens[token.address].amount = amount;
    onNumberChange2 && onNumberChange2();
  };
...
var Dashboard = (props) => {
  ...
  const poolTokenNumeraire = { name: "Pool AssetToken", symbol: "P", price: 1 };
  const [numeraire, setNumeraire] = (0, import_react2.useState)(poolTokenNumeraire);
  ...
  const price = (address2) => {
    ...
    return asset && asset.weight && asset.reserve ? poolTokens * asset.weight / asset.reserve : 0;
  };
  const balance = (address2) => {
    const lowerAddress = address2.toLowerCase();
    return assetTokens[lowerAddress] ? assetTokens[lowerAddress].balance : 0;
  };
  const numeraires = [poolTokenNumeraire, ...(_a = Object.values(assetTokens)) == null ? void 0 : _a.map((a) => ({ name: a.name, symbol: a.symbol, price: price(a.address) }))];
  const numeraireMap = {};
...
//# sourceMappingURL=/build/_shared/chunk-LI7MLTFT.js.map
