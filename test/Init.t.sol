...
contract InitTest is PRBTest {
    ...
    function testUninitialize() public {
        address alice = address(1);
        vm.startPrank(alice);
        Pool pool = new Pool("Pool", "P");
        ...
        // test uninitialize gives you complete amount back
        assertEq(tokenA.balanceOf(alice), 0);
        pool.uninitialize(0);
        assertEq(tokenA.balanceOf(alice), amountA);
        vm.stopPrank();

        // test that non-owner can not uninitialize
        ...
        // test uninitialize returns complete amount
        vm.startPrank(alice);
    ...
    /// @dev check that uninitialize can not be called after initialization
    function testUninitializeAfterInit() public {
        address alice = address(1);
        vm.startPrank(alice);
        Pool pool = new Pool("Pool", "P");
        ...
        // test that owner can not uninitialize after intialization
        vm.expectRevert(abi.encodeWithSelector(Pool.AlreadyInitialized.selector));
        pool.uninitialize(0);
        vm.stopPrank();
