...
struct Asset {
    ...
    //     return asset(token).scale.div(_scale);
    // }
        ...
        // Compute fracValueIn
        {
            for (uint256 i; i < payTokens.length; i++) {
                ...
                if (payToken == address(this)) {
                    _balance -= amountIn;
                    fracValueIn += amountIn.div(_balance);
                } else {
                    Asset storage assetIn = _assets[_index[payToken]];
                    uint256 gamma = ONE - assetIn.fee;
                    uint256 weightIn = assetIn.scale.div(_scale);
                    fracValueIn += gamma.mul(weightIn).mul(amountIn).div(
        ...
        // Compute receiveAmounts
        {
            ...
            for (uint256 i; i < receiveTokens.length; i++) {
                ...
                if (receiveToken == address(this)) {
                    factor = fracValueIn.mul(allocation);
                    amountOut = _balance.mul(factor).div(ONE - factor);
                    receiveAmounts[i] = amountOut;
                    _balance += amountOut;
                } else {
                    Asset storage assetOut = _assets[_index[receiveToken]];
                    uint256 gamma = ONE - assetOut.fee;
                    uint256 weightOut = assetOut.scale.div(_scale);
                    factor = gamma.div(weightOut).mul(allocation).mul(
                        fracValueIn
                    );
                    amountOut = assetOut.balance.mul(factor).div(
                        factor + ONE
                    );
                    receiveAmounts[i] = amountOut;
        ...
        // Transfer tokens to the pool
        {
            for (uint256 i; i < payTokens.length; i++) {
                ...
                if (payToken == address(this)) {
                    ...
                    delta = _assets[_index[payToken]].fee.mul(amount);
            ...
            for (uint256 i; i < receiveTokens.length; i++) {
                ...
                if (receiveToken == address(this)) {
                    ...
                    delta = _assets[_index[receiveToken]].fee.mul(amountOut);
    ...
    function swap(
        ...
        {
            uint256 gamma = (ONE - assetIn.fee).mul(
                ONE - assetOut.fee
            );
            uint256 weightRatio = assetIn.scale.div(assetOut.scale);
            uint256 invGrowthOut = ONE +
                gamma.mul(weightRatio).mul(amountIn.div(reserveIn));
            reserveOut = assetOut.balance.div(invGrowthOut);
        ...
        uint256 deltaIn = assetIn.fee.mul(amountIn);
        uint256 deltaOut = assetOut.fee.mul(amountOut);
    ...
    function stake(
...

        {
            uint256 gamma = ONE - assetIn.fee;
            uint256 invGrowthOut = ONE -
                gamma.mul(weightIn).mul(amountIn.div(reserveIn));
            reserveOut = _balance.div(invGrowthOut);
        ...
        uint256 delta = assetIn.fee.mul(amountIn);
        assetIn.scale += delta;
        _scale += delta;
    ...
    function unstake(
...

        {
            uint256 gamma = ONE - assetOut.fee;
            uint256 invGrowthOut = ONE +
                gamma.div(weightOut).mul(amountIn).div(reserveIn);
            reserveOut = assetOut.balance.div(invGrowthOut);
            amountOut = assetOut.balance - reserveOut;
        }
        ...
        uint256 delta = assetOut.fee.mul(amountOut);
    ...
    function _stakeBatch(
        ...
        for (uint256 i; i < n; i++) {
            ...
            {
                uint256 invGrowthOut = ONE -
                    amountIn[i].mul(weightIn).div(reserveIn);
                amountOut += reserveOut.div(invGrowthOut) - reserveOut;
            }
    ...
    */
    ...
    function addAsset(
...

        {
            ...
            uint256 valueIn = reserve0.mul(assetIn.scale).div(_scale);
            scaleIn = valueIn.mul(amountIn).div(reserveIn);
        }
